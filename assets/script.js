jQuery(document).ready(function($) {
    ...
    function renderQRCode(url) {
        ...
        if (typeof QRCode !== 'undefined') {
            // Use the URL (which is now the UPI deep link) for QR code
            new QRCode(qrBox, {
                text: url, // Use the UPI deep link received from the server
                width: 220,
                height: 220,
                colorDark: '#000000',
                colorLight: '#ffffff',
            ...
            $('#cod-payment-status').text('Scan QR with UPI app. ₹1 will be refunded after payment.'); // You might want to update this message
        } else {
            console.error("QRCode.js library not loaded.");
            // Fallback: Use online QR code generator with the UPI deep link
            const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(url)}`; // Use the UPI deep link
    ...
    function renderMobileRedirect(url) {
        ...
        if (proceedBtn) {
            proceedBtn.onclick = function() {
                // Use the URL (which is now the UPI deep link) for mobile redirect
                window.location.href = url; // Redirect to the UPI deep link received from the server
            };
        }

        $('#cod-payment-status').text('You will be redirected to UPI app.'); // You might want to update this message
    }
